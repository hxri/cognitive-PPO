...
class Agent(nn.Module):
    ...
    def get_value(self, x, appraisal, sts):
        ...
        xe = (x2 - torch.min(x2)) / (torch.max(x2) - torch.min(x2))
        # xr = torch.cat([x1, appraisal, sts], dim=-1)
        cr = self.actor(xe)
    ...
    def get_action_and_value(self, x, appraisal, sts, action=None):
        x1 = self.conv(x.permute(0, 3, 1, 2))
        xe = x1 # torch.cat([x1, appraisal], dim=-1)
        # if(args.monitor_only):
        #     xe = x1
        # else:
        # xr = torch.cat([x1, appraisal], dim=-1)
        x2 = torch.mean((x1.unsqueeze(2) * appraisal.unsqueeze(1)), dim=2)
        xr = (x2 - torch.min(x2)) / (torch.max(x2) - torch.min(x2))
        logits = self.actor(xr)
        napp = appraisal_calc(x[:][..., 0], self.actor(xr))  
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            sts[step] = stress(app)
            rewards[step] = rw # + 0.1 * (torch.mean((torch.mean(app, 0) - torch.tensor([1.0, 0.0, 1.0, 1.0, 0.0, 1.0])), dim=-1))# - sts[step].squeeze(1)
            # print(rewards[step])
            appraisals[step] = app
            next_appraisal = app
            next_sts = sts[step]
