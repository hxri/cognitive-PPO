...
def make_env(gym_id, seed, idx, capture_video, run_name):
    def thunk():
        # env = gym.make(gym_id)
        env = gym.make(gym_id, render_mode="rgb_array", max_steps=max_steps, agent_view_size=agent_view_size, n_obstacles=n_obstacles, size=size, agent_start_pos=agent_start_pos)
...
class Agent(nn.Module):
    ...
    def get_appraisal(self, x):
        ...
        mot = motivational_relevance(x[:][..., 0])
        cert = certainity(xe)
        app = torch.stack((mot, cert), -1)
        # print(app)
        return app
    ...
    def get_action_and_value(self, x, appraisal, action=None):
        ...
        cert = torch.tensor(certainity(logits))
        napp = torch.stack((mot, cert), -1)
...
if __name__ == "__main__":
    ...
    appraisals = torch.zeros((args.num_steps, args.num_envs) + (2,)).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        # print(appraisals[0])
        b_appraisal = appraisals.reshape((-1,) + (2,))
        # print(b_appraisal[0])
        # print(b_obs.shape)
