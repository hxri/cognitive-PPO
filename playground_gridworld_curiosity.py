...
# Curiosity model (Forward dynamics model)
class CuriosityModel(nn.Module):
    def __init__(self, envs):
        super(CuriosityModel, self).__init__()

        self.encoder = nn.Sequential(
            nn.Linear(64 * 7 * 7 + envs.single_action_space.n, 256),
            nn.ReLU(),
            nn.Linear(256, 64),
            nn.ReLU()
        )
    ...
    def forward(self, state, action):
        encoded = torch.cat((state.view(state.size(0), -1), action), dim=1)
        encoded = self.encoder(encoded)
        decoded = self.decoder(encoded)
        decoded = decoded.view(decoded.size(0), 64, 7, 7)
        return decoded
...
def calculate_curiosity_reward(observation, action, next_observation):
        observation = observation.permute(0, 3, 1, 2)
        next_observation = next_observation.permute(0, 3, 1, 2)

        # Calculate prediction error using the curiosity model
        predicted_next_observation = CuriosityModel(observation, action, next_observation)
        prediction_error = torch.mean((next_observation - predicted_next_observation)**2)

        # Scale the prediction error to obtain the curiosity reward
        curiosity_reward = -prediction_error  # Negative error as curiosity is maximized
