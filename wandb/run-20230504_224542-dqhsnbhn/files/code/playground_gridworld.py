...
def make_env(gym_id, seed, idx, capture_video, run_name):
    def thunk():
        # env = gym.make(gym_id)
        max_steps = 50
        env = gym.make(gym_id, render_mode="rgb_array", size=10)
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
                ...
                for item in info['final_info']:
                    if(item):
                        ...
                        if 'episode' in item.keys():
                            ...
                            ereturn.append(item['episode']['r'])
                            elength.append(item['episode']['l'])
                            break
        ...
        writer.add_scalar("charts/episodic_return", np.mean(ereturn), global_step)
        writer.add_scalar("charts/episodic_length", np.mean(elength), global_step)
